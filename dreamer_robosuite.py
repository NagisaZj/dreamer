...
def define_config():
  ...
  config.controller='JOINT_VELOCITY'
  config.robot1='Panda'
  config.robot2='None'
  config.gripper='PandaGripper'
...
class Dreamer(tools.Module):
  ...
  def _build_model(self):
    ...
    self._encode = models.DenseEncoder(42,2, self._c.num_units, act=act)
    self._dynamics = models.RSSM(
        self._c.stoch_size, self._c.deter_size, self._c.deter_size)
    # self._decode = models.ConvDecoder(self._c.cnn_depth, cnn_act)
    self._decode = models.DenseDecoder2(42, 2, self._c.num_units, act=act)
...
def make_env(config, writer, prefix, datadir, store):
  ...
  if suite == 'dmc':
    ...
    env = robosuite.make(
      task,
      robots=robots,  # load a Sawyer robot and a Panda robot
      gripper_types=config.gripper,  # use default grippers per robot arm
      controller_configs=controller_config,  # each arm is controlled using OSC
      env_configuration="default",  # (two-arm envs only) arms face each other
      has_renderer=False,  # no on-screen rendering
      has_offscreen_renderer=False,  # no off-screen rendering
      control_freq=config.control_freq,  # 20 hz control for applied actions
